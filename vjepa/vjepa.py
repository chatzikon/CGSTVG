...
class VJepa:
    ...
    def run_one_step(self, iteration, sample, training=True):
        if training:
            
            import pdb
            pdb.set_trace()

            new_epoch = iteration / self.ipe
            if new_epoch != self.current_epoch:
                self.current_epoch = new_epoch
                self.criterion = torch.nn.CrossEntropyLoss()
                self.top1_meter = AverageMeter()
        results = run_one_step(
            device=self.device,
            training=training,
            num_temporal_views=self.eval_num_segments if self.attend_across_segments else 1,
            attend_across_segments=self.attend_across_segments,
            num_spatial_views=1,
...
def run_one_step(
    ...
    with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
        ...
        clips = [
            ...
            for di in sample[0]  # iterate over temporal index of clip
        ]
        clip_indices = [d.to(device, non_blocking=True) for d in sample[2]]
    ...
    if iteration % 20 == 0:
        logger.info('[%5d] %.3f%% (loss: %.3f) [mem: %.2e]'
                    % (iteration, top1_meter.avg, loss,
                        torch.cuda.max_memory_allocated() / 1024.**2))

    return {"accuracy": top1_meter.avg, "outputs": outputs}
